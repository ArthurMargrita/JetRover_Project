...
class GoalPublisherApp(QMainWindow):
    def __init__(self):
        ...
        #form_layout.addRow(QLabel("pi/6 = 0.52; pi/4 = 0.78; pi/3 = 1.03; pi/2 = 1.57")
    ...
    def pose_callback(self, msg):
        """
        Callback pour recevoir la position actuelle du robot depuis /amcl_pose.
        """
        self.robot_position = (msg.pose.pose.position.x, msg.pose.pose.position.y)

    def check_positions(self, event):
        """
        Vérifie toutes les secondes si la position actuelle du robot est proche du goal.
        """
        if self.goal_position and self.robot_position:
            ...
            # Vérifier si le robot est arrivé à la position cible avec une tolérance de 0.2
            if distance <= 0.2:
                rospy.loginfo("Le robot a atteint la position cible !")
                self.goal_position = None  # Réinitialiser la position cible après l'arrivée

    def stop(self):

		"""
		Annule le goal en cours en publiant sur /move_base/cancel.
		"""
		rospy.loginfo("Annulation du goal en cours...")
		
		# Création du publisher pour le topic /move_base/cancel
		cancel_pub = rospy.Publisher('/move_base/cancel', GoalID, queue_size=10)
		
		# Attendre un instant que le publisher soit prêt
		rospy.sleep(1)
		
		# Publier un message GoalID vide pour annuler tous les goals
		cancel_msg = GoalID()
