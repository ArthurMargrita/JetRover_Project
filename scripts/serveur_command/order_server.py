...
class OrderServer:
    ...
    # Toutes les méthodes manquantes ajoutées ici
    def get_first(self):
        self.log("Demande du premier élément")
        if self.order_list:
            return jsonify(self.order_list[0]), 200
        return 'Order list is empty', 404

    def get_last(self):
        self.log("Demande du dernier élément")
        if self.order_list:
    ...
    def remove_first(self):
        self.log("Suppression du premier élément")
        if self.order_list:
    ...
    def remove_last(self):
        self.log("Suppression du dernier élément")
        if self.order_list:
            self.order_list.pop(-1)
            return 'Last item removed', 200
        return 'Order list is empty', 404
