...
class OrderReceptionState(EventState):
    ...
    Si le numéro de table (la première partie avant le point-virgule) vaut "1" ou "2",
    il crée une PoseStamped fixe :
        - pour la table 1 : position x=3, y=0, z=0 et orientation (0,0,0,1)
        - pour la table 2 : position x=0, y=-2, z=0 et orientation (0,0,0,1)
    Il efface la commande via l'endpoint /clear_list et renvoie l'outcome 'done'.
    
    Dans le cas contraire, il affiche un message indiquant que la commande est destinée 
    à une table autre que 1 ou 2 et renvoie l'outcome 'failed'.
    ...
    Outcomes:
        done    La commande concerne la table 1 ou 2.
        failed  La commande concerne une table autre que 1 ou 2.
    '''
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not self._order_received:
                    ...
                    if table_number == '1':
                        # Crée le message PoseStamped pour la table 1
                        goal_pose = PoseStamped()
                        goal_pose.header.stamp = rospy.Time.now()
                        goal_pose.header.frame_id = "map"
                        goal_pose.pose.position = Point(-4.0, -1.5, 0.0)
                        ...
                        # Crée le message PoseStamped pour la table 2
                        goal_pose = PoseStamped()
                        goal_pose.header.stamp = rospy.Time.now()
                        goal_pose.header.frame_id = "map"
                        goal_pose.pose.position = Point(3.0, 1.0, 0.0)
                        goal_pose.pose.orientation = Quaternion(0.0, 0.0, 0.0, 1.0)
                        
                        userdata.goal_pose = goal_pose
                        Logger.loginfo("Commande pour table 2 reçue. Objectif : %s" % str(goal_pose))
                        ...
                        Logger.logwarn("Commande reçue pour la table %s (attendu: 1 ou 2)" % table_number)
                        return 'failed'
            else:
