...
class JoyToCmdVel:
    ...
    def joy_callback(self, data):
        ...
        if self.mode:  # Mode déplacement
            # Détection des axes actifs (ignorant les retours à zéro suspects)
            for i in range(len(self.last_axes)):
                if abs(data.axes[i]) > 0.1:  # Seuil pour éviter les faux zéros
